...
void Config::readMinefieldCornersFromTopic(const visualization_msgs::MarkerArray::ConstPtr & corners)
{
    ...
    Corners_and_dimension.data.resize(corners->markers.size()*2 + 6);
...
//        visualization_msgs::Marker& m = corners->markers[i];
    ...
    // Convert minefield corners in relation to tf/minefield frame
        ...
        for(int i = 0; i < minefieldCorners.size(); i++)
        {
            ...
            minefieldCorners[i][0] = pointOut.point.x;//cout << pointOut.point.x <<"     " << pointOut.point.y << endl;
            minefieldCorners[i][1] = pointOut.point.y;
            minefieldCorners[i][2] = pointOut.point.z;
            int indx = 6 + i*2;
            Corners_and_dimension.data[indx] = minefieldCorners[i].x();
			Corners_and_dimension.data[indx+1] = minefieldCorners[i].y();
        }
   ...
   // }
    // Find boundaries
        lowerBound=upperBound=minefieldCorners[0];//(T==0?minefieldCorners[0]:minefieldCorners[4]);
        //for(int i = (T==0?0:4); i < (T==0?4:minefieldCorners.size()); ++i)
        for(int i = 0; i < minefieldCorners.size(); ++i)
        {
        ...
        Corners_and_dimension.data[0] = centerx;
        Corners_and_dimension.data[1] = centery;
